...
public class UserDAO extends CassandraData {
  ...
  private static UserDAO getUserWithQuorum(String username) {
	  ...
	  Statement s = QueryBuilder.select().from("users").where(QueryBuilder.eq("username", username)).setConsistencyLevel(ConsistencyLevel.ALL);
...
*/
    ...
    Row userRow = getSession().execute(s).one();    // TODO - replace the "<something>" with the right thing.
